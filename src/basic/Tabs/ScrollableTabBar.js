...
const ScrollableTabBar = createReactClass({
  ...
  updateTabUnderline(position, pageOffset, tabCount) {
    ...
    if (position < tabCount - 1) {
      if (this.props.isRtl) {
        const nextTabLeft = this._tabsMeasurements[position + 1].right;
        const nextTabRight = this._tabsMeasurements[position + 1].left;

        const newLineLeft =
          pageOffset * nextTabLeft + (1 - pageOffset) * lineRight;
        const newLineRight =
          pageOffset * nextTabRight + (1 - pageOffset) * lineLeft;
      } else {
